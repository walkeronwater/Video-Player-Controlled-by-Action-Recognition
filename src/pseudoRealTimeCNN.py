...
folderPath = os.path.abspath('./DataSet/newFromRealTime/zjh323')
            ...
            if not valid:
                ...
                if recordTimesCounter >= winWidth:  # means reading is over
                    ...
                    if isPlot:
                        ...
                        plt.plot(tools.generateX(winWidth),
                                 smoothSig[0], color='red', label='channel 1')
                        plt.plot(tools.generateX(winWidth),
                                 smoothSig[1], color='orange', label='channel 2')
                        plt.plot(tools.generateX(winWidth),
                                 smoothSig[2], color='blue', label='channel 3')
                        plt.legend()
                        plt.xlabel('time')
                        plt.ylabel('value')
                        plt.show()
